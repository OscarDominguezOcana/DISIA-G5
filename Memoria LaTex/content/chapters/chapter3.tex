...
Entre las características con mayor puntuación de información mutua para la tarea de clasificación destacan: \texttt{overall\_match\_score}, \texttt{same\_love\_language}, \texttt{ambition\_diff}, \texttt{similar\_ambition}, \nolinkurl{emotional\_expressiveness\_diff} y \texttt{personality\_distance\_sum}. Estos resultados son coherentes con el análisis exploratorio del Capítulo~\ref{cap:datos}, que ya señalaba la homofilia en el lenguaje del amor y la proximidad en ambición como factores determinantes.
...
Para abordar el problema de predicción de compatibilidad, se han seleccionado distintos modelos de aprendizaje automático con el objetivo de comparar cómo se comportan enfoques diferentes sobre el mismo conjunto de datos. Se parte de un modelo lineal como la regresión logística, para evaluar si las variables generadas durante la ingeniería de características son suficientes para explicar la compatibilidad. A partir de ahí, se incluyen modelos más complejos como árboles de decisión, métodos ensemble (Random Forest y Gradient Boosting) y KNN, que permiten capturar relaciones no lineales, interacciones complejas y similitudes entre perfiles.

Los hiperparámetros se han fijado siguiendo valores habituales en la práctica, buscando configuraciones razonables sin excesiva complejidad. En árboles se han limitado parámetros como la profundidad para evitar sobreajuste, en KNN se ha utilizado un número reducido de vecinos y en regresión logística una configuración estándar. No se ha realizado una optimización exhaustiva, ya que el objetivo es comparar modelos de forma equilibrada. Como mejora futura, sería interesante ajustar los hiperparámetros de manera más sistemática, especialmente en modelos más sensibles como Decision Tree, Random Forest, Gradient Boosting y KNN.
...
Para la predicción de la longevidad de la relación se han utilizado las versiones de regresión de los modelos empleados en clasificación, manteniendo así coherencia y facilitando la comparación entre ambos enfoques. En concreto, se han incluido modelos como Ridge, árboles de decisión, Random Forest, Gradient Boosting y KNN en su versión de regresión, con el objetivo de analizar cómo se comportan estas mismas familias al predecir una variable continua como la duración de la relación.

Los hiperparámetros se han ajustado de forma similar a la tarea de clasificación, utilizando valores conservadores para evitar configuraciones demasiado complejas. No se ha realizado una optimización exhaustiva, ya que el objetivo principal es comparar modelos de manera equilibrada. Como mejora futura, sería interesante estudiar el impacto de un ajuste más detallado, especialmente en modelos como Random Forest, Gradient Boosting y KNN.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.55\textwidth]{Memoria LaTex/Pictures/matriz_validación.png}
  \caption{Matriz de confusión en validación}
  \label{fig:matriz_validación}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.55\textwidth]{Memoria LaTex/Pictures/matriz_test.png}
  \caption{Matriz de confusión en test}
  \label{fig:matriz_test}
\end{figure}

\subsection{Importancia relativa de variables por modelo en clasificación}

La Figura~\ref{fig:importancia_variables_clasificacion} muestra la importancia relativa de las variables en los distintos modelos de clasificación. Se observa que \texttt{same\_love\_language} es la variable más relevante en todos los modelos, lo que indica que compartir el mismo lenguaje afectivo tiene un fuerte impacto en la compatibilidad. 

También destacan variables relacionadas con diferencias entre individuos, como \nolinkurl{emotional\_expressiveness\_diff}, \texttt{ambition\_diff} y \texttt{spontaneity\_diff}, especialmente en modelos no lineales. Esto sugiere que la compatibilidad depende en gran medida de la interacción entre perfiles.

En general, la importancia está distribuida entre varias variables, lo que refleja que se trata de un problema multifactorial.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.80\textwidth]{Pictures/importancia_variables_clasificacion.png}
  \caption{Importancia relativa de las variables en los modelos de clasificación. Los valores están normalizados por modelo y permiten comparar la relevancia de cada característica en la predicción de la compatibilidad.}
  \label{fig:importancia_variables_clasificacion}
\end{figure}
...
\subsection{Importancia relativa de variables por modelo en regresión}

La Figura~\ref{fig:importancia_variables_regresion} muestra la importancia relativa de las variables en los distintos modelos de regresión. A diferencia del caso de clasificación, no se observa una variable dominante. La importancia se distribuye entre múltiples características.

Destacan variables como \texttt{overall\_match\_score}, \texttt{same\_love\_language} y diferencias en rasgos individuales como \texttt{emotional\_expressiveness\_diff} o \texttt{spontaneity\_diff}, especialmente en modelos no lineales como Random Forest y Gradient Boosting.

Sin embargo, la variabilidad en la importancia entre modelos y la ausencia de patrones consistentes reflejan la dificultad del problema y la limitada capacidad explicativa de las variables disponibles.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.80\textwidth]{Pictures/importancia_variables_regresion.png}
  \caption{Importancia relativa de las variables en los modelos de regresión. Los valores están normalizados por modelo y muestran una distribución más homogénea de la relevancia, sin variables claramente dominantes.}
  \label{fig:importancia_variables_regresion}
\end{figure}
